...
def create_post(create_post_req: CreatePostReq, admin_id: AdminDep, conn: DBDep):
    with conn.cursor(row_factory=class_row(Post)) as cur:
        ...
        record = cur.execute(
            """insert into posts (user_id, category_id, title, content, status, published_at, updated_at)
                    values (%(user_id)s,%(category_id)s,%(title)s,%(content)s,%(status)s,%(published_at)s,%(updated_at)s)
                    returning *""",
            params,
        ).fetchone()
        return record
